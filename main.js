...
class LivingAIAssistant {
    constructor() {
        console.log("Initializing Living AI Assistant...");
        this.scene = null;
        this.camera = null;
        this.renderer = null;
        ...
        this.assistantPersonality = {
            ...
            speakingRate: 0.9,
            pitch: 1.1,
            personality: "friendly, helpful, and slightly mystical"
        };
    ...
    async init() {
        try {
            ...
            this.loadPresets();
            this.loadUIState();
            console.log("Living AI Assistant initialization complete");
        } catch (error) {
    ...
    setupControls() {
        ...
        if (speakBtn) {
            speakBtn.addEventListener('click', () => {
                const input = prompt('What would you like to ask Nova?');
                if (input && input.trim()) {
                    this.processUserInput(input.trim());
                }
            });
        }
...
const app = new LivingAIAssistant();
