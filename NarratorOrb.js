...
class NarratorOrb {
    constructor(scene, camera, renderer, analyserNode = null, config = {}) {
        ...
        this.attentionLevel = 0; // 0-1, how focused the AI is
    ...
    setListeningState(isListening) {
        ...
        this.attentionLevel = isListening ? 0.8 : 0.2;
        
        // Visual feedback for listening state
        if (this.nebulaMaterial?.uniforms) {
            this.nebulaMaterial.uniforms.intensity.value = isListening ? 2.5 : 1.8;
        }
    }
    
    setSpeakingState(isSpeaking) {
        this.isSpeaking = isSpeaking;
        this.attentionLevel = isSpeaking ? 1.0 : 0.2;
        
        // Visual feedback for speaking state
        if (this.nebulaMaterial?.uniforms) {
            this.nebulaMaterial.uniforms.intensity.value = isSpeaking ? 3.0 : 1.8;
        }
        
        // Add pulsing effect when speaking
        if (isSpeaking && this.orbGroup) {
            this.orbGroup.scale.setScalar(1.2);
        }
    }
    
    setAttentionLevel(level) {
        this.attentionLevel = Math.max(0, Math.min(1, level));
        
        // Adjust visual intensity based on attention
        const baseIntensity = 1.8;
        const attentionMultiplier = 1 + this.attentionLevel * 0.5;
        
        if (this.nebulaMaterial?.uniforms) {
            this.nebulaMaterial.uniforms.intensity.value = baseIntensity * attentionMultiplier;
        }
    }
